...
class TeamController extends Controller
{
    ...
    {
        $type = DB::select("SELECT COLUMN_TYPE FROM information_schema.`COLUMNS` WHERE TABLE_NAME = 'teams' AND COLUMN_NAME = 'jabatan'")[0]->COLUMN_TYPE;
    ...
    public function store(Request $request)
    {
        try {
            $request->validate([
                'fullname' => 'required',
                'position' => 'required|unique:teams,jabatan',
                'images' => 'required|max:5020|mimes:png,jpg'
            ]);
            $fileName = Auth()->user()->username . now()->format('d-M-Y') . time() . '.' . $request->file('images')->getClientOriginalName();
            $request->file('images')->storeAs('public/uploads/images', $fileName);
            $team = new Team();
